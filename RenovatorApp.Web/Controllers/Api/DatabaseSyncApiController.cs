...
public sealed class DatabaseSyncApiController : ControllerBase
{
    ...
    public async Task<ActionResult<DatabaseSyncResponse>> GetDatabase(CancellationToken cancellationToken)
    {
        ...
        var employees = await _dbContext.Employees.AsNoTracking().ForCompany(renoCompanyID).OrderBy(employee => employee.GivenName).ThenBy(employee => employee.FamilyName).Select(employee => new DatabaseSyncEmployeeDto(employee.EmployeeId, employee.GivenName, employee.FamilyName, employee.PrimaryPhone, employee.PrimaryEmailAddress, employee.IsInspector, employee.IsDefaultInspector, employee.InspectorHourlyRate)).ToListAsync(cancellationToken);
        ...
        var calendarEvents = await _dbContext.CalendarEvents.AsNoTracking().ForCompany(renoCompanyID).OrderBy(calendarEvent => calendarEvent.Date).ThenBy(calendarEvent => calendarEvent.StartTime).Select(calendarEvent => new DatabaseSyncCalendarEventDto(calendarEvent.CalendarEventId, calendarEvent.RenoUserID, calendarEvent.Title, calendarEvent.Date, calendarEvent.AllDay, calendarEvent.StartTime, calendarEvent.EndTime, calendarEvent.EventAlertTimes, calendarEvent.Notes, calendarEvent.IsPrivate, calendarEvent.IsDeleted, calendarEvent.InspectionId, calendarEvent.CreatedAtUtc, calendarEvent.UpdatedAtUtc)).ToListAsync(cancellationToken);
        var customerProperties = await _dbContext.Customers.AsNoTracking().ForCompany(renoCompanyID).SelectMany(customer => customer.Properties.Select(property => new DatabaseSyncCustomerPropertyDto(customer.CustomerId, property.PropertyId))).ToListAsync(cancellationToken);
        ...
        return new DatabaseSyncResponse(DateTime.UtcNow, addresses, buildings, buildingTypes, customers, calendarEvents, customerProperties, employees, properties, parts, partSources, inspectionAreaCategories, inspectionAreaTypes);
    }
...
public sealed record DatabaseSyncResponse(DateTime PulledAtUtc, IReadOnlyList<DatabaseSyncAddressDto> Addresses, IReadOnlyList<DatabaseSyncBuildingDto> Buildings, IReadOnlyList<DatabaseSyncBuildingTypeDto> BuildingTypes, IReadOnlyList<DatabaseSyncCustomerDto> Customers, IReadOnlyList<DatabaseSyncCalendarEventDto> CalendarEvents, IReadOnlyList<DatabaseSyncCustomerPropertyDto> CustomerProperties, IReadOnlyList<DatabaseSyncEmployeeDto> Employees, IReadOnlyList<DatabaseSyncPropertyDto> Properties, IReadOnlyList<DatabaseSyncPartDto> Parts, IReadOnlyList<DatabaseSyncPartSourceDto> PartSources, IReadOnlyList<DatabaseSyncInspectionAreaCategoryDto> InspectionAreaCategories, IReadOnlyList<DatabaseSyncInspectionAreaTypeDto> InspectionAreaTypes);
...
public sealed record DatabaseSyncCalendarEventDto(Guid UniqueEventId, Guid RenoUserID, string Title, DateTime Date, bool AllDay, TimeSpan StartTime, TimeSpan EndTime, string EventAlertTimes, string Notes, bool IsPrivate, bool IsDeleted, Guid? InspectionId, DateTime CreatedAtUtc, DateTime UpdatedAtUtc);
public sealed record DatabaseSyncCustomerPropertyDto(Guid CustomerId, Guid PropertyId);
public sealed record DatabaseSyncEmployeeDto(Guid Id, string FirstName, string LastName, string Phone, string Email, bool IsInspector, bool IsDefaultInspector, decimal InspectorHourlyRate);
