...
public sealed class DatabaseSyncApiController : ControllerBase
{
    ...
    public async Task<ActionResult<DatabaseSyncResponse>> GetDatabase(CancellationToken cancellationToken)
    {
        ...
        var properties = await _dbContext.Properties.AsNoTracking().ForCompany(renoCompanyID).OrderBy(property => property.PropertyId).Select(property => new DatabaseSyncPropertyDto(property.PropertyId)).ToListAsync(cancellationToken);
